        ...
        "doors": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "w":    {"type": "string", "description": "幅W。数値（例 '1324'）または『特注』などの文字"},
                    "h":    {"type": "string", "description": "高さH。数値（例 '2035'）または『特注』などの文字"}
                },
...
PROMPT = (
    "これは住宅の建具表（設計仕様詳細）の画像です。表に並ぶ建具を1行ずつ漏れなく読み取り、"
    "次の項目を抽出してください：建具番号(no, 例 WD101)、種類(type, 例 片引戸/片開きドア/折戸)、"
    "幅W(w)、高さH(h)、分かれば場所/部屋(room)。\n"
    "・幅・高さは『W1324』『H2035』のような表記から数字部分（例:'1324','2035'）を取り出してください。\n"
    "・ただし『特注』『W特注』『H特注』など数字でない記載がある場合は、その文字（例:'特注'）を w/h にそのまま入れてください。空文字にしないでください。\n"
    "・w と h は別々の列です。両方とも必ず返してください（同じ建具で h だけ抜け落ちないように注意）。\n"
    "・項目が見当たらない時のみ空文字にしてください。\n"
    "・表にあるすべての建具を上から順に返してください。"
)
